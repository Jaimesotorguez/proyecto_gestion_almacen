...
<html>
<head>
    <title>Listado liquidacion</title>
    <meta charset="utf-8">
...
<body>
    ...
    <script>
        ...
        $(document).ready(function() {
                ...
                $.ajax({
                  url: 'obtener_ids_liquidaciones.php',
                  method: 'GET',
                  
                  success: function(response) {
                    // Parsear la respuesta JSON
                    
                    var liquidaciones = JSON.parse(response);
                    // Crear una nueva fila para la tabla
                    
                    liquidaciones.forEach(function(liquidacion) {
                        var fila = '<tr>' +
                            '<td>' + liquidacion.id + '</td>' +
                            '<td>' + liquidacion.n_pedido + '</td>' +
                            '<td>' + liquidacion.cliente + '</td>' +
                            '<td>' + liquidacion.metodo + '</td>' +
