...
if(isset($_POST['submit'])){
    ...
    header("Location: consultar_ruta.php?id_cliente=".$id_cliente);
    exit;
}
...
<body>
	...
	<div class="container">
		<div class="form-container">
			<h2 class="mb-4">Buscar Ruta</h2>
			<form method="POST" action="">
    			<div class="form-group form-row">
    				<label class="col-sm-3" for="id_cliente">Ruta</label>
    				<div class="col-sm-9">
    				    <select class="form-control" id="id_cliente" name="id_cliente">
                            <option value="">Selecciona un ruta</option>
                            <?php
                                // Consulta para obtener los clientes
                                $consulta_clientes = "SELECT id FROM rutas";
                                ...
                                while($fila_cliente = mysqli_fetch_assoc($resultado_clientes)){
                                    echo '<option value="'.$fila_cliente['id'].'">'.$fila_cliente['id'].'</option>';
                                }
                            ?>
                        </select>
    				</div>
