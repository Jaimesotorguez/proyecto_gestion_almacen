...
if(isset($_POST['submit'])){
    ...
    header("Location: consultar_pedido.php?id_cliente=".$id_cliente);
    exit;
}
...
<body>
	...
	<div class="container">
		<div class="form-container">
			<h2 class="mb-4">Buscar Pedido</h2>
			<form method="POST" action="">
    			<div class="form-group form-row">
    				<label class="col-sm-3" for="id_cliente">Pedido</label>
    				<div class="col-sm-9">
    				    <select class="form-control" id="id_cliente" name="id_cliente">
                            ...
                            <?php
                                ...
                                {
                                    $consulta_clientes = "SELECT id, n_pedido FROM pedidos WHERE id_cliente =".$_SESSION['cliente_id'];
                                }
                                else
                                {
                                    $consulta_clientes = "SELECT id, n_pedido FROM pedidos";
                                }
                                ...
                                while($fila_cliente = mysqli_fetch_assoc($resultado_clientes)){
                                    echo '<option value="'.$fila_cliente['id'].'">'.$fila_cliente['n_pedido'].'</option>';
                                }
                            ?>
                        </select>
    				</div>
